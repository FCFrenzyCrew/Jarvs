...
exports.run = async (client, message, args) => {
        if(!args[0]) {
                const embed = new Discord.RichEmbed()
                        .setDescription(`<a:success:730737395774914620> Lütfen geçerli bir sayı belirt dostum!`)
                        .setColor("RANDOM")
                        .setTimestamp()
  ...
  if (!mentionedChannel && args[0] !== "sıfırla") return message.channel.send(s1);
        ...
        if(args[0] === "sıfırla") {
                ...
                const embed = new Discord.RichEmbed()
                        .setDescription(`<a:success:730737395774914620> Sayaç, Sıfırlandı Dostum!`)
                        .setColor("RANDOM")
                        .setTimestamp()
                message.channel.send({embed})
                return
        ...
        if(isNaN(args[0])) {
                const embed = new Discord.RichEmbed()
                        .setDescription(`<a:9876_attention:730738007233265725> Lütfen, geçerli bir sayı belirtiniz!`)
                        .setColor("RANDOM")
                        .setTimestamp()
                message.channel.send({embed})
                return
  ...
  profil[message.guild.id].kanal = mentionedChannel.id
        ...
        const embed = new Discord.RichEmbed()
                .setDescription(`<a:success:730737395774914620> Sayaç, başarılı bir şekilde \`${args[0]}\` olarak ayarlandı, sayaç kanalı ise ${mentionedChannel} olarak ayarlandı!`)
                .setFooter('Jarvıs!', client.user.avatarURL)
                .setColor("RANDOM")
                .setTimestamp()
        message.channel.send({embed})
}
