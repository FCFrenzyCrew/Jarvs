...
exports.run = (client, message, args) => {
  ...
  if (!message.guild.members.get(client.user.id).hasPermission("BAN_MEMBERS")) return message.reply('Bu Komutu Kullanmaya İzniniz Yok!')
  ...
  if (message.mentions.users.size < 1) return message.reply('Sunucunuzdan Yasaklamamı İstediğiniz Kullanıcıyı Etiketleyin!');
  if (reason.length < 1) return message.reply('Lütfen Yasaklama Nedenini Belirt!');
  ...
  .setColor("RANDOM")
  .setDescription(`Seçilen Kullanıcıyı Başarıyla Yasakladım!`)
